...
class Db_Pixels {
	 ...
	 */
	public static function insert_pixels( array $pixels, string|null $force_source = null, bool|null $force_ownership = null ): int {
		global $wpdb;
	...
	public static function get_all_pixels(
		...
		if ( $multiple === true ) {
			$where_add .= " AND (SELECT COUNT(post_id) > 1 FROM $table_pixels_posts WHERE public_identification_id = pixels.public_identification_id) = 1";
		}

		if ( $public_identification_id ) {
			$where_add .= $wpdb->prepare( " AND pixelposts.public_identification_id = %s", $public_identification_id );
		}
		...
		$result = $wpdb->get_results( "
				SELECT 
					...
					(SELECT COUNT(post_id) > 1 FROM $table_pixels_posts WHERE public_identification_id = pixels.public_identification_id) AS multiple,
				    posts.ID as post_id,
				...
				WHERE
				    NOT (pixelposts.public_identification_id IS NOT NULL AND posts.ID IS NULL)
				AND 
				    (posts.post_type != 'revision' OR posts.ID IS NULL)
				{$where_add}
				{$orderby_add}
		", ARRAY_A );
	 ...
	 * @return bool
	 */
	public static function remove_pixel_from_post( string $public_identification_id, int $post_id ): bool {
		global $wpdb;
	 ...
	 * @return bool | int
	 */
	public static function disable_pixel( string $public_identification_id ): bool|int {
		global $wpdb;
	...
	public static function assign_pixel_to_post( string $public_identification_id, int $post_id ): int|bool {
		...
		$result = $wpdb->insert( $table_pixel_posts,
			array(
				'public_identification_id' => $public_identification_id,
				'post_id'                  => $post_id,
				'active'                   => true,
	...
	public static function set_pixel_activation_status( string $public_identification_id, bool $activated = true, int|null $post_id = null ): bool {
		...
		$result = $wpdb->update( $table_pixel_posts,
