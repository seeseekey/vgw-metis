...
class Setup {
	...
	public function activate(): void {
		if ( ! $this->create_table() ) {
			\wp_die( \esc_html__( 'Bei der Erstellung der Datenbank-Tabelle ist ein Fehler aufgetreten', 'vgw-metis' ) );
		}
	...
	public function create_table(): bool {
		...
		if ( version_compare( $db_version, $plugin_version, '<' ) ) {
			...
			// Add metis_pixels table
			$create_metis_pixels = "CREATE TABLE `{$wpdb->base_prefix}metis_pixels` (
			...
			dbDelta($create_metis_pixels);
			if ( !empty( $wpdb->last_error ) ) {
				error_log( "Error creating table metis_pixels: " . $wpdb->last_error );
			}

			// Add metis_pixel_posts table
			$create_metis_pixel_posts = "CREATE TABLE `{$wpdb->base_prefix}metis_pixel_posts` (
				public_identification_id varchar(100),
			...
			dbDelta( $create_metis_pixel_posts );
			if ( !empty( $wpdb->last_error ) ) {
				error_log( "Error creating table metis_pixels: " . $wpdb->last_error );
			}

			// Add metis_participants table
			$create_metis_participants = "CREATE TABLE `{$wpdb->base_prefix}metis_participants` (
			...
			dbDelta( $create_metis_participants );
			if ( !empty( $wpdb->last_error ) ) {
				error_log( "Error creating table metis_participants: " . $wpdb->last_error );
			}

			// Add metis_text_limit_changes table
			$create_metis_text_limit_changes = "CREATE TABLE `{$wpdb->base_prefix}metis_text_limit_changes` (
				post_id int NOT NULL,
				public_identification_id varchar(100),
				changed_at datetime DEFAULT null,
				text_length int NOT NULL
			) $charset_collate;";
			dbDelta( $create_metis_text_limit_changes );
			if ( !empty( $wpdb->last_error ) ) {
				error_log( "Error creating table metis_text_limit_changes: " . $wpdb->last_error );
			}

			$success = empty( $wpdb->last_error );
			\update_site_option( 'metis_db_version', $plugin_version );
		}
