...
class Page_Message extends Page {
    ...
    /**
     * @var object | null ai disclaimer to display
     */
    public object|null $ai_disclaimer = null;

    /**
     * @var bool | null previous rejected ai disclaimer used to mark the try of reporting text written with ki
     */
    public bool|null $previous_rejected_ai_disclaimer = null;
	...
	public function load_data(): void {
		...
		$errorMsgText = empty($_GET['custom_text']) ? null : $_GET['custom_text'];

        if ($errorMsgText && str_contains($errorMsgText, '66')) { // as we redirect after a error occurs on requests
            $this->previous_rejected_ai_disclaimer = true;
        }
	}
    ...
    public function render(): void
    {
        ...
        $this->load_ai_disclaimer();

		if($this->pixel->text_type != Common::TEXT_TYPE_LYRIC) {
	...
	public function show_text_length_limit_warning(): void {
		...
		if ( $current_text_length < Common::DEFAULT_TEXT_LENGTH_MIN) {
			...
			if ( $text_length_limits && $text_length_limits->default_text_limit_reached !== true ) {
			...
			if ( $text_length_limits && $text_length_limits->long_text_limit_reached !== true ) {
	...
	public function save_message(): void {
        ...
        if($form_data->aiDisclaimerAnswer &&  ($form_data->previousRejectedaiDisclaimer === null)) {
            $form_data->previousRejectedaiDisclaimer = false;
        }
        ...
        if (!isset($form_data->aiDisclaimerAnswer)) {
            wp_redirect(admin_url('admin.php?page=metis-message&post_id=' . $this->post_id . '&notice=wp_metis_save_message_no_ai_disclaimer_answer'));
            exit;
        }
        ...
        $message->aiDisclaimerAnswer      = $form_data->aiDisclaimerAnswer;
        $message->previousRejectedAiDisclaimer = $form_data->previousRejectedaiDisclaimer;
		...
		foreach ( $form_data->participants as $participant ) {
			...
			if ( get_current_user_id() === $participant->id ) {
				...
				$new_participant->firstName   = $participant->first_name ? $participant->first_name : null;
		...
		if (!isset($message->involvement)) {
			$message->involvement = "NO_PARTICIPATION";
		}
	...
	private function submit_message( $payload ): void {
		...
		try {
            ...
            if(!empty($error_response_body->message->errorcode)) {
                $error_code = $error_response_body->message->errorcode;
                if($error_code === 66) {
                    $this->previous_rejected_ai_disclaimer = true;
                }
            }
    ...
    /**
     * Calls API to load text for ai disclaimer
     *
     * @return void
     * @throws GuzzleException
     */
    public function load_ai_disclaimer(): void
    {
        $this->ai_disclaimer = Tom_Pixels::load_ai_disclaimer();

        if (empty($this->ai_disclaimer) || !$this->ai_disclaimer->text || !$this->ai_disclaimer->yesChoice || !$this->ai_disclaimer->noChoice) {
            $this->error_message = 'Es wurde kein Text zur Erklärung für KI-Systeme gefunden.';
        }
    }
    ...
    public static function register_notifications(Notifications &$notifications): void
    {
        ...
        $notifications->add_notice_by_key('wp_metis_save_message_no_ai_disclaimer_answer', esc_html__('Fehler: Sie müssen eine Erklärung zur Verwendung von KI-Systemen abgeben.', 'vgw-metis'));
